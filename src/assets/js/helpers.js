import { subset, row, index, matrix, parse } from 'mathjs';
...
function latexifyMatrix(
    ...
    for (let r = 0; r < rows; ++r) {
        if (cols > 1) {
            matrixSmall.push(subset(row(matrix_, r), index(0, _.range(cols))).valueOf()[0]);
        } else {
            matrixSmall.push([row(matrix_, r)]);
        }
    }

    matrixSmall = matrix(matrixSmall);
    ...
    let matrixStr = matrix(matrixSmall).toString();
    let matrixTex = parse(matrixStr)
        .toTex()
        .replaceAll("null", "")
        .replace("-999", "\\dots")
        .replace("999", "\\vdots")
